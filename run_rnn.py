...
def run(args):
    ...
    model = RNN(name=args.name, stock_id=args.stock_id,
                ...
                stateful=True, input_size=args.input_size,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rnn_activation', required=False, default="relu", type=str)
    parser.add_argument('--dense_output_size', required=False, default=[], type=int, nargs='+',)
    parser.add_argument('--dense_activation', required=False, default="relu", type=str)
    ...
    if args.stock_id.lower() == "all":
        ...
        for stock_id in stocks_list:
            ...
            tf.print("="*100)
            tf.print("START TO TRAIN {}".format(args.stock_id))
            run(args)
            tf.print("="*100)
    else:
        run(args)
